...
public class Shop {
		...
		private static Shop from(Row row) {
	        return new Shop(row.getLong("id"), row.getString("name") , row.getString("address"), row.getString("postalCode") );
	    }
	    ...
	    public static Uni<Shop> findById(MySQLPool client, Long id) {
	        return client.preparedQuery("SELECT id, name, address, postalCode FROM Shops WHERE id = ?").execute(Tuple.of(id)) 
	                .onItem().transform(RowSet::iterator) 
	                .onItem().transform(iterator -> iterator.hasNext() ? from(iterator.next()) : null); 
	    }
	    
	    public Uni<Boolean> save(MySQLPool client , String name_R, String address_R, String postalCode_R)  {
	        return client.preparedQuery("INSERT INTO Shops(name,address,postalCode) VALUES (?,?,?)").execute(Tuple.of(name_R , address_R , postalCode_R))
	    ...
	    public static Uni<Boolean> update(MySQLPool client, Long id_R, String name_R, String address_R, String postalCode_R) {
	        return client.preparedQuery("UPDATE Shops SET name = ?, address = ?, postalCode = ? WHERE id = ?").execute(Tuple.of(name_R,address_R,postalCode_R,id_R))
	        		.onItem().transform(pgRowSet -> pgRowSet.rowCount() == 1 ); 
	    }  
}
