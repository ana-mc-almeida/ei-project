...
public class Loyaltycard {
	...
	public static Multi<Loyaltycard> findById(MySQLPool client, Long id) {
		return client.preparedQuery("SELECT id, idCustomer, idShop FROM LoyaltyCards WHERE id = ? ORDER BY idShop ASC")
				.execute(Tuple.of(id))
				.onItem().transformToMulti(set -> Multi.createFrom().iterable(set))
				.onItem().transform(Loyaltycard::from);
	}
