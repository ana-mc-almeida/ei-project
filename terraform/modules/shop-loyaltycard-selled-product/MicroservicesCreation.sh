...
sudo docker pull "${docker_username}"/"${module_name2}":1.0.0-SNAPSHOT
sudo docker run -d\
  --name "${module_name2}"\
  -p 8081:8081\
  -e QUARKUS_DATASOURCE_USERNAME="${db_username}" \
  ...
  -e QUARKUS_DATASOURCE_REACTIVE_URL="mysql://${rds_address}:${rds_port}/${db_name}" \
  -e KAFKA_BOOTSTRAP_SERVERS="$kafka" \
   "${docker_username}"/"${module_name2}":1.0.0-SNAPSHOT

sudo docker pull "${docker_username}"/"${module_name3}":1.0.0-SNAPSHOT
sudo docker run -d\
  --name "${module_name3}"\
  -p 8082:8082\
  -e QUARKUS_DATASOURCE_USERNAME="${db_username}" \
  ...
  -e QUARKUS_DATASOURCE_REACTIVE_URL="mysql://${rds_address}:${rds_port}/${db_name}" \
  -e KAFKA_BOOTSTRAP_SERVERS="$kafka" \
   "${docker_username}"/"${module_name3}":1.0.0-SNAPSHOT
