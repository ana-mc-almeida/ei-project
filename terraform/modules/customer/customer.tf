...
resource "aws_instance" "deployQuarkusCustomer" {
  ...
  tags = {
    Name = "terraform-deploy-QuarkusCustomers"
  }
}
...
variable "security_group_name" {
  ...
  type        = string
  default     = "terraform-Quarkus-customers"
}

resource "null_resource" "docker_build" {
  provisioner "local-exec" {
    command = "docker login -u \"${var.docker_image_user}\" -p \"${var.docker_image_create_token}\" && cd ../microservices/customer && ./mvnw clean package -Dquarkus.container-image.group=${var.docker_image_user} -Dquarkus.docker.buildx.platform=linux/arm64,linux/amd64 -Dquarkus.container-image.push=true"
  }
}

output "customerAddress" {
  value       = aws_instance.deployQuarkusCustomer.public_dns
